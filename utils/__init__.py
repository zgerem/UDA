...
def extract_ampl_phase_new(fft_im):
    # fft_im: size should be bx3xhxw
    fft_amp, fft_pha = fft_im.abs(), fft_im.angle()
    return fft_amp, fft_pha
...
def low_freq_mutate_new(amp_src, amp_trg, L=0.1):
...
def FDA_source_to_target_old(src_img, trg_img, L=0.1):
    # exchange magnitude
...
def FDA_source_to_target(src_img, trg_img, L=0.1):
    ...
    # replace the low frequency amplitude part of source with that from target
    amp_src_ = low_freq_mutate_new( amp_src, amp_trg, L=L )

    # recompose fft of source
    #fft_src_ = amp_src_* torch.exp( 1j * pha_src )
    fft_src.real = torch.cos(pha_src.clone()) * amp_src_.clone()
    fft_src.imag = torch.sin(pha_src.clone()) * amp_src_.clone()
    src_in_trg = torch.fft.ifft2(fft_src, dim=(-2,-1))
